  ...
  const load = useCallback(async () => {
    ...
    const built = await Promise.all(rooms.map(async (room) => {
      ...
      const [techView, offer, messages] = await Promise.all([
        technicianRepositoryV2.getViewForCompany(room.technicianId, DEMO_COMPANY_ID),
        ...
        {entries.length === 0 && (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptySub}>
              Chat becomes available when you accept a technician's application or they accept your direct offer.
            </Text>
          </View>
        )}

        {entries.map(({ room, techDisplay, offerTitle, lastMessage }) => (
          <TouchableOpacity
            key={room.id}
            style={styles.card}
          ...
          >
            <View style={styles.cardTop}>
              <View style={styles.avatar}>
                <Text style={styles.avatarText}>{techDisplay.charAt(0).toUpperCase()}</Text>
              </View>
              <View style={styles.cardBody}>
                ...
                <Text style={styles.preview} numberOfLines={1}>
                  {lastMessage
                    ? (lastMessage.senderRole === 'company' ? 'You: ' : '') + lastMessage.body
                    : 'No messages yet'}
                </Text>
              </View>
            </View>
...
const styles = StyleSheet.create({
  ...
  techName: { fontSize: 14, fontWeight: '700', color: colors.text, flex: 1 },
  time: { fontSize: 11, color: colors.textMuted, flexShrink: 0, paddingLeft: spacing.xs },
  offerLine: { fontSize: 11, color: colors.blue, fontWeight: '600', marginBottom: 2 },
