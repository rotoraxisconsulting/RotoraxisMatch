...
-- Returns the company_id of the calling company_user (assumes one primary company)
...
**Core privacy rules:**
- A company can always read the public fields (anonymous_code, age, technician_type, city, country, verification_status, etc.).
...
- RLS on the base table allows company reads of verified technicians (row-level access).
- Column-level privacy is enforced by `technician_public_view`, which uses CASE WHEN to gate private fields behind `offer_accepted_between()`.
...
In Supabase, RLS does not selectively reveal columns — policies are row-level. To protect individual columns, use a **view** that enforces column-level privacy:
...
-- Company users can read any verified, active technician profile
-- (column-level privacy enforced via view — see below)
CREATE POLICY tp_select_company ON technician_profiles
  FOR SELECT USING (
...
Documents are locked by default. A company can read a technician's documents only after an accepted offer.

```sql
ALTER TABLE documents ENABLE ROW LEVEL SECURITY;

-- Technician reads/writes their own documents
...
CREATE POLICY docs_update_own ON documents
  FOR UPDATE USING (technician_id = my_technician_id());

-- Company can read only if documents_unlocked = true in an accepted offer record
...
-- Admin can read and update all documents (for verification)
...
The frontend may only write `status` (to accept, reject, or withdraw).
`identity_revealed`, `documents_unlocked`, and chat room creation are handled exclusively by the `handle_offer_accepted` trigger.
...
|-------|-------|---------------------|
| offer_requests | status (accept / reject / withdraw only) | ✓ |
| offer_requests | identity_revealed | **NO** — trigger only |
| offer_requests | documents_unlocked | **NO** — trigger only |
| offer_applications | status (accept / reject / withdraw only) | ✓ |
...
The frontend sends exactly one thing: `PATCH /offer_requests/{id}` with `{ status: 'accepted' }`.
The server does the rest atomically.

### Trigger: on_offer_accepted

Runs on BEFORE UPDATE on both `offer_requests` and `offer_applications`.
Fires when `status` transitions to `'accepted'` from any other value.

```sql
-- Pseudocode — full implementation in Phase V2-9
CREATE OR REPLACE FUNCTION handle_offer_accepted()
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.status = 'accepted' AND OLD.status <> 'accepted' THEN
    -- 1. Unlock identity and documents atomically
    NEW.identity_revealed  := true;
    NEW.documents_unlocked := true;

    -- 2. Create the chat room (idempotent)
    --    For offer_requests:
    INSERT INTO chat_rooms (offer_request_id, technician_id, company_id)
    VALUES (NEW.id, NEW.technician_id, NEW.company_id)
    ON CONFLICT DO NOTHING;

    --    For offer_applications (handled in a separate trigger function):
    --    INSERT INTO chat_rooms (offer_application_id, technician_id, company_id)
    --    VALUES (NEW.id, NEW.technician_id, NEW.company_id)
    --    ON CONFLICT DO NOTHING;
  END IF;

  -- Prevent client from directly writing identity_revealed or documents_unlocked
  -- by resetting them if status is not accepted
...
| documents | read/write own | only if documents_unlocked | read/update all |
