// ============================================================
// LOCAL DEMO ONLY — NOT SUPABASE SEEDS
// ============================================================
// This module loads V2 JSON seed data into AsyncStorage for local
// demo/development mode.
//
// ID STRATEGY — LOCAL vs SUPABASE:
//   Local demo IDs:  human-readable strings (tech-001, comp-001,
//                    prof-t001, offer-001, oreq-001, room-seed-001, ...)
//   Supabase IDs:    real UUIDs generated by Postgres (gen_random_uuid())
//                    profiles.id must equal auth.users.id
//
// These local seed IDs are intentionally NOT UUIDs. They are designed
// for readability during local development and must NOT be inserted into
// a Supabase database. Supabase will generate its own UUIDs at runtime.
//
// The Supabase migration starts CLEAN:
//   1. Run SUPABASE_SCHEMA_V2.sql (enums + catalog tables only)
//   2. Run docs/V2_S1_ADMIN_BOOTSTRAP_SQL.sql (first admin account)
//   3. Register real users via Supabase Auth — IDs come from auth.uid()
//   Do NOT import or replay any local JSON seeds into Supabase.
// ============================================================
...
export const localDatabase = {
  ...
  async repairV2DemoData(): Promise<void> {
    const staleApplicationKeys = new Set([
      'oapp-001|tech-002|offer-004|comp-003',
      'oapp-003|tech-001|offer-001|comp-001',
      'oapp-005|tech-014|offer-007|comp-005',
    ]);
    const staleRoomId = 'room-seed-002';
    const staleMessageIds = new Set(['msg-seed-004', 'msg-seed-005', 'msg-seed-006']);

    const applications = await storageAdapter.get<Array<{
      id?: string;
      technicianId?: string;
      offerId?: string;
      companyId?: string;
      status?: string;
    }>>(DB_KEYS.v2OfferApplications) ?? [];

    const nextApplications = applications.filter((application) => {
      const key = `${application.id}|${application.technicianId}|${application.offerId}|${application.companyId}`;
      const isStaleSeed = staleApplicationKeys.has(key);

      return !isStaleSeed;
    });

    const removedStaleApplication = nextApplications.length !== applications.length;
    if (removedStaleApplication) {
      await storageAdapter.set(DB_KEYS.v2OfferApplications, nextApplications);
    }

    const rooms = await storageAdapter.get<Array<{
      id?: string;
      offerApplicationId?: string;
    }>>(DB_KEYS.v2ChatRooms) ?? [];
    const nextRooms = rooms.filter(
      (room) => !(room.id === staleRoomId && room.offerApplicationId === 'oapp-003'),
    );
    if (nextRooms.length !== rooms.length) {
      await storageAdapter.set(DB_KEYS.v2ChatRooms, nextRooms);
    }

    const messages = await storageAdapter.get<Array<{
      id?: string;
      chatRoomId?: string;
      senderId?: string;
      senderUserId?: string;
      ...
      senderRole?: string;
    }>>(DB_KEYS.v2ChatMessages) ?? [];
    const filteredMessages = messages.filter(
      (message) => message.chatRoomId !== staleRoomId && !staleMessageIds.has(message.id ?? ''),
    );

    const technicianProfileById = new Map(
      v2TechnicianProfiles.map((technician) => [technician.id, technician.userId]),
    );
    const companyMemberByCompanyId = new Map(
      v2CompanyMembers
        .filter((member) => member.role === 'admin')
        .map((member) => [member.companyId, member]),
    );

    const nextMessages = filteredMessages.map((message) => {
      if (message.senderUserId || !message.senderId) return message;
      const { senderId, ...rest } = message;
      if (message.senderRole === 'technician') {
        return {
          ...rest,
          senderUserId: technicianProfileById.get(senderId) ?? senderId,
        };
      }
      if (message.senderRole === 'company') {
        const member = companyMemberByCompanyId.get(senderId);
        return {
          ...rest,
          senderUserId: member?.userId ?? senderId,
          senderCompanyMemberId: member?.id ?? message.senderCompanyMemberId,
        };
      }
      return message;
    });

    const migratedMessages = nextMessages.some(
      (message, index) => message !== filteredMessages[index],
    );
    if (nextMessages.length !== messages.length || migratedMessages) {
      await storageAdapter.set(DB_KEYS.v2ChatMessages, nextMessages);
    }
  },
